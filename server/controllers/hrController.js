...
export const dashboard = async (req, res, next) => {
  try {
    ...
    const [totalEmployees, todayAttendance, onLeave, monthly, monthlyHours] =
      await Promise.all([
    ...
    const averageWorkingMinutes = monthlyHours[0]?.records
      ? monthlyHours[0].totalMinutes / monthlyHours[0].records
      : 0;
...
export const employees = async (req, res, next) => {
  try {
    ...
    const totals = Object.fromEntries(
      summary.map((item) => [String(item._id), item.workingMinutes || 0]),
    );
  ...
  try {
    ...
    if (approved) {
      ...
      if (!user) {
        ...
        return res
          .status(400)
          .json({ message: "Employee does not have enough leave balance" });
      }
    }
