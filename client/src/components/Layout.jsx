  ...
  return (
    <div className="min-h-screen lg:flex">
      <button
        ...
        className="fixed left-4 top-4 z-20 rounded-lg bg-slate-900 p-2 text-white lg:hidden"
      >
        <Menu size={20} />
      </button>
      <aside
        className={`fixed inset-y-0 left-0 z-30 flex w-64 flex-col bg-slate-900 p-5 text-slate-300 transition-transform lg:static lg:translate-x-0 ${open ? "translate-x-0" : "-translate-x-full"}`}
      >
        ...
        <nav className="space-y-2">
          {links.map(({ to, label, icon: Icon }) => (
            <NavLink
              end={to === "/employee" || to === "/hr"}
              key={to}
              to={to}
              onClick={close}
              className={({ isActive }) =>
                `flex items-center gap-3 rounded-lg px-3 py-3 ${isActive ? "bg-brand-600 text-white" : "hover:bg-slate-800 hover:text-white"}`
              }
            >
              <Icon size={19} />
              {label}
      ...
      <main className="min-w-0 flex-1 p-5 pt-20 lg:p-8">{children}</main>
    </div>
  );
}
